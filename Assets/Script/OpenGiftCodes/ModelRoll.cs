...
public class ModelRoll : MonoBehaviour
{
    [Header("Source")]
    public CaseOpeningScroll scrollSource;
    ...
    private bool missingScrollSourceWarningLogged;

    private void Awake()
    {
        if (content == null)
        {
            Debug.LogError("[ModelRoll] content is NULL (assign in inspector).", this);
            return;
        }
    ...
    private void OnEnable()
    {
        if (scrollSource != null)
            scrollSource.OnRollNamesReady += HandleRollNamesReady;
        else
            LogMissingScrollSourceWarning();
    }
    ...
    private void HandleRollNamesReady(List<string> rollNames)
    {
        names.Clear();
        percents.Clear();
        ...
        if (rollNames != null)
        {
            for (int i = 0; i < rollNames.Count; i++)
            {
                var n = rollNames[i];
                ...
                float p = scrollSource != null ? scrollSource.GetChancePercentByName(n) : -1f;
    ...
    public void StartRoll()
    {
        ...
        if (names.Count == 0)
        {
            ...
            if (scrollSource != null && scrollSource.LastRollNames != null && scrollSource.LastRollNames.Count > 0)
            {
                HandleRollNamesReady(new List<string>(scrollSource.LastRollNames));
                pendingRoll = false;
            }
    ...
    private void LogMissingScrollSourceWarning()
    {
        if (missingScrollSourceWarningLogged)
            return;

        missingScrollSourceWarningLogged = true;
        Debug.LogWarning("[ModelRoll] CaseOpeningScroll not found. Assign 'scrollSource' in inspector.", this);
    }
